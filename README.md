...
- ⚙️ **Админ-панель** (`/admin`, доступна только твоему Telegram ID):
  ...
  - 📢 **рассылка** — присылаешь боту любое сообщение (текст, фото и т.п.), бот показывает
    превью и после подтверждения отправляет его всем, кто хоть раз пользовался ботом;
  - 📊 **статистика** — сколько всего человек заходило, сколько новых за сегодня/7 дней,
    сколько активных за 7 дней и сколько раз открывали каждый раздел.

Все данные хранятся в `content.json` (контент) и `users.json` (список пользователей для
рассылки и статистики) рядом с ботом — редактировать руками не нужно, всё меняется через
